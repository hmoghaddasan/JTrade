...
"""
پیاده‌سازی provider sms.ir
مستندات: https://sms.ir/rest-api/

SMS.IR از روش named parameters برای قالب‌های Verify پشتیبانی می‌کند.
"""
...
class SmsIrProvider(BaseSmsProvider):
    ...
    ERROR_CODES = {
        ...
        116: 'متغیرهای ارسالی جهت قالب اشتباه می‌باشد',
    }
    ...
    # ============================================
    # 3. ارسال OTP با پارامترهای نام‌دار (send/verify)
    # ============================================
    def send_otp_multi(
        self, mobile: str, params: List[Dict[str, str]], client_reference_id: str = None
    ) -> SmsResult:
        """
        ارسال OTP با پارامترهای نام‌دار
        params: لیست دیکشنری [{'name': 'CODE', 'value': '1234'}, ...]
        """
        if not self.otp_template_id:
            return SmsResult(
                success=False,
                error_message='قالب OTP (otp_template_id) تنظیم نشده',
                status=MessageStatus.FAILED,
            )
        ...
        logger.info(f"📤 [SmsIr] send_otp_multi payload: {payload}")
    ...
    def check_status(self, message_id: str) -> DeliveryStatus:
        ...
        if result.get('status') != 1:
            return DeliveryStatus(
                ...
                error_message=result.get('message', 'خطا در دریافت وضعیت'),
                raw_response=result,
            )
    ...
    # ============================================
    # 5. دریافت پیام‌های ورودی (receive/live)
    # ============================================
    def get_inbox(
        ...
        if result.get('status') != 1:
            logger.warning(f"[SmsIr] Inbox fetch failed: {result.get('message')}")
            return []
        ...
        for item in items:
            try:
                ...
                logger.error(f"[SmsIr] Error parsing inbox item: {e}")
